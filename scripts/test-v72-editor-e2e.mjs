/**
 * FORMA V7.2 — Verified Pure UI Editor E2E Test Suite
 * 
 * Strict Hardening:
...
for (const d of [outputsDir, screenshotsDir, fixturesDir, exportedPdfsDir, logsDir, testResultsDir]) {
  if (!fs.existsSync(d)) fs.mkdirSync(d, { recursive: true });
}

// Synchronous log flushing to avoid buffer drop on exit
const logFilePath = path.join(logsDir, 'test-v72-editor-e2e.log');
...
// Perform pure UI export by clicking toolbar "Dışa aktar" and dialog "Dosyayı indir"
async function triggerPureUiExport(cdp) {
  ...
  // Clear previous exported PDF pointer in test environment
  await cdp.evaluate(`window.__lastExportedPdf = null;`);
  ...
  // Wait for export processing to complete and retrieve exported bytes
  let exportedBytes = null;
  for (let i = 0; i < 60; i++) {
    const raw = await cdp.evaluate(`
      (() => {
        if (window.__lastExportedPdf && window.__lastExportedPdf.length > 0) {
          return Array.from(window.__lastExportedPdf);
        }
        return null;
      })()
    `);
    if (raw) {
      exportedBytes = new Uint8Array(raw);
      break;
    }
    await new Promise((r) => setTimeout(r, 250));
  }

  assert.ok(exportedBytes && exportedBytes.length > 500, 'Exported PDF bytes not captured via UI save()');
  log(`    [UI Export] Successfully exported PDF via UI (${exportedBytes.length} bytes)`);
  return exportedBytes;
}

async function runE2ESuite() {
  log('================================================================');
  log('  FORMA PDF V7.2 — VERIFIED PURE UI E2E TEST SUITE');
  log('================================================================\n');

  const profileDir = getTempDir('chrome_v72_e2e_');
  ...
  try {
    ...
    fs.writeFileSync(path.join(fixturesDir, 'scanned-v72-fixture.pdf'), samplePdfBytes);
    log(`✓ Fixture saved to ${path.join(fixturesDir, 'scanned-v72-fixture.pdf')} (${samplePdfBytes.length} bytes)`);
    ...
    await cdp.evaluate(`
      ...
      const file = new File([bytes], "scanned-v72-fixture.pdf", { type: "application/pdf" });
    ...
    const postCancelPdfBytes = await triggerPureUiExport(cdp);
    ...
    const updatedText = "Name: Sukru Yildiz - Verified V7.2";
    ...
    // Trigger export through pure UI buttons (NO test API exportPdfCurrent)
    const exportedPdfBytes = await triggerPureUiExport(cdp);
    const exportedPdfPath = path.join(exportedPdfsDir, 'forma-v72-exported.pdf');
    fs.writeFileSync(exportedPdfPath, exportedPdfBytes);
    const exportedPdfLength = exportedPdfBytes.length;
    const finalExportB64 = Buffer.from(exportedPdfBytes).toString('base64');
    ...
    await cdpNew.evaluate(`
      ...
      const bytes = new Uint8Array(binary.length);
      for (let i = 0; i < binary.length; i++) bytes[i] = binary.charCodeAt(i);
      const file = new File([bytes], "forma-v72-reopened.pdf", { type: "application/pdf" });
      const input = document.querySelector('input[type="file"]');
      if (input) {
        const dt = new DataTransfer();
        dt.items.add(file);
    ...
    fs.writeFileSync(path.join(testResultsDir, 'v72-e2e-summary.json'), JSON.stringify(summary, null, 2), 'utf8');
