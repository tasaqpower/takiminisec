...
const outputsDir = path.join(projectRoot, 'outputs', 'v6-e2e');
if (!fs.existsSync(outputsDir)) {
  fs.mkdirSync(outputsDir, { recursive: true });
}

console.log('================================================================================');
console.log('  FORMA V6 AUDIT: REAL CHROMIUM CDP E2E INTERACTION & GEOMETRY ACCEPTANCE');
console.log('================================================================================\n');
...
async function createSampleV6Pdf() {
  ...
  // Corporate logo (3:1 aspect ratio)
  const logoPng = createPng(120, 40, 30, 60, 140, 255);
  const logoImage = await doc.embedPng(logoPng);

  // Raster watermark image (faint red)
  const watermarkPng = createPng(180, 70, 220, 70, 70, 80);
  const watermarkImage = await doc.embedPng(watermarkPng);

  // Page 1
  const page = doc.addPage([600, 800]);

  // Repeating Logo in header
  ...
  // Document Title
  page.drawText('FORMA V6 AUDIT & ACCEPTANCE PROTOCOL', {
    x: 185,
    y: 755,
    size: 15,
    font: fontHelvetica,
  ...
  // Typewriter Monospace Target (vector text)
  ...
  page.drawText('Bu belge Forma V6 gercek kullanici etkilesim ve geometri testleri icin tanzim edilmistir.', {
    x: 50,
    y: 590,
    size: 11,
    font: fontHelvetica,
  ...
  // Embedded raster watermark image
...
async function main() {
  const profileDir = path.join(process.env.TEMP || 'C:/Users/sinan/AppData/Local/Temp', 'chrome_v6_e2e_' + Date.now());
  ...
  console.log('3. Generating and uploading sample V6 PDF fixture...');
  const samplePdfBytes = await createSampleV6Pdf();
  ...
  await cdp.evaluate(`
    ...
    const file = new File([bytes], "forma-v6-test.pdf", { type: "application/pdf" });
  ...
  console.log(`  OCR completed. Extracted ${ocrResults[0]?.lines?.length || 0} line(s) with confidence ${ocrResults[0]?.confidence || 0}%.`);
  ...
  // Double click and apply modification
  await cdp.evaluate(`
    (() => {
      const orig = {
        id: "orig-sukru-accepted",
        page: 0,
        x: 50,
        y: 670,
        w: 180,
        ...
        bold: true,
        isOcr: true,
        ocrOriginalBounds: { x: 50, y: 670, w: 180, h: 22 },
        ocrBackgroundColor: "#ffffff"
      };
      // Convert with move to x=90, y=690 and updated text
      window.__formaTestApi.convertOriginalToMark(orig, {
        text: "Name: Sukru Yildiz (Verified)",
        x: 90,
        y: 690,
        ocrTextDirty: true
      }, false);
  ...
  const activeMark = finalState.marks.find(m => m.kind === 'text' && (m.sourceId === 'orig-sukru-accepted' || m.id.includes('orig-sukru-accepted')));
