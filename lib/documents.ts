...
export type Mark = {id:string; page:number; kind:"text"|"draw"|"highlight"|"signature"; x:number; y:number; w:number; h:number; color:string; size:number; text?:string; image?:string; points?:{x:number;y:number}[]};
...
export async function exportPdf(bytes:Uint8Array,pages:PageItem[],marks:Mark[]){
 const src=await PDFDocument.load(bytes),out=await PDFDocument.create();out.registerFontkit(fontkit);
 const font=marks.some(m=>m.kind==="text")?await out.embedFont(await (await fetch("/fonts/Roboto-Regular.ttf")).arrayBuffer(),{subset:true}):null;
 const renderer=marks.length?await loadPdf(bytes):null;
 try{for(const item of pages){const [page]=await out.copyPages(src,[item.index]);out.addPage(page);const originalRotation=page.getRotation().angle;
  ...
  if(annotations.length&&renderer){const original=await renderer.getPage(item.index+1);const unit=original.userUnit||1;const viewport=original.getViewport({scale:1});const point=(x:number,y:number)=>viewport.convertToPdfPoint(x,y);
   for(const m of annotations){
    if(m.kind==="text"&&font){const [x,y]=point(m.x,m.y+m.size);page.drawText(m.text||"",{x,y,size:m.size/unit,font,color:col(m.color),rotate:degrees(originalRotation)})}
